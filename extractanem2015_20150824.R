...
files = vector('list', length=1)
len = numeric(1)
files[[1]] = list.files(path = 'gpx', pattern="^Track.*gpx")
len[1] = length(files[[1]])

# create a dataframe to hold all GPS data
infile = readGPXGarmin(paste('gpx/', files[[1]][1], sep='')) # read in first file to get column names
