...
surv = read.csv("GPSSurveys2015.surveyinfo.csv") # data on surveys
options(digits=8)
...
for(j in 1:length(files)){ # loop through each GPS
	for(i in 1:len[j]){ # loop over each file in this folder
		...
		if(length(jj)>0){
			k = which(intimes >= starttimePXs[[j]][jj] & intimes <= endtimePXs[[j]][jj]) # find the GPX points that fit within the survey
			outfile = list(header = infile$header, data = infile$data[k,])
			outfile$data$elev = 0 # set elevation to 0
			writeGPX(filename = paste('gpx_trimmed/GPS', j, '_', files[[j]][i], sep=''), outfile = outfile)
		}
	}
}
