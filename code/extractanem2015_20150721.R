...
# NOTE: this code probably double counts from fish. Fish seen in both visits will get counted twice.
# MAYBE BETTER TO ONLY COPY OVER THE NEWLY TAGGED FISH? TAKE THE OBSERVATION WITH THE MOST UNTAGGED FISH?
...
if(length(dups)>0){
	for(i in 1:length(dups)){
		...
		if(length(inds)<3){
			# get anemone data from the second visit
			oldanemids = unique(data$oldAnemID[inds][!is.na(data$oldAnemID[inds])])
			if(length(oldanemids) > 1) stop('multiple oldanemIDs for the same anemone')
			if(length(oldanemids) ==1) data$oldAnemID[inds[1]] = oldanemids

				# could average anemone size and location data here
				# should also check the species ID to make sure they match
	
			# get fish data from the second visit to the anemone
			...
			inds2 = (!is.na(newsizes) & newsizes != '') | (newcols != '') | (!is.na(newids) & newids != '') # which entries have some data
			newsizes = as.numeric(newsizes[inds2])
			newcols = newcols[inds2]
			newids = as.numeric(newids[inds2])
